...
이 문서는 Claude Apps Gateway와 inference rate-limit proxy를 EKS에 배포하기 위한 패키징, Kubernetes 리소스, 네트워크 정책, 운영 확인 절차를 정의한다. Gateway 설정 자체는 [apps-gateway-config.md](./apps-gateway-config.md)를 따른다.
...
- `Deployment`: `llm-rate-limit-proxy`
- `Service`: rate-limit proxy ClusterIP, port 8080
...
- rate-limit proxy Service `llm-rate-limit-proxy.llm-gateway.svc.cluster.local:8080`
- RDS PostgreSQL endpoint
- OIDC issuer/token/userinfo endpoints
- 필요 시 OTLP collector
- Kubernetes DNS

rate-limit proxy egress:
...
5. 모델 요청 시 rate-limit proxy와 vLLM Pod log에 `/v1/messages` 요청이 찍히는지 확인한다.

   ```sh
   kubectl -n llm-gateway logs deploy/llm-rate-limit-proxy
   kubectl -n vllm logs deploy/vllm
   ```

6. rate limit을 초과하는 burst 요청이 `429`로 차단되고 vLLM까지 전달되지 않는지 확인한다.

7. Gateway Pod에서 외부 Anthropic API로 직접 나갈 수 없는지 확인한다.
...
8. vLLM Service를 일시적으로 중단했을 때 Gateway가 외부 Anthropic API로 failover하지 않고 upstream 오류를 반환하는지 확인한다.
...
| vLLM log에 요청 없음 | `base_url`, rate-limit proxy log, NetworkPolicy egress, Service DNS, Service selector |
...
- 모델 요청은 rate-limit proxy를 거쳐 vLLM `/v1/messages`로 전달된다.
