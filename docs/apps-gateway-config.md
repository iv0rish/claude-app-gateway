...
이 문서는 Claude Apps Gateway가 사내 vLLM endpoint를 사용하도록 구성하는 기준을 정의한다. 배포 패키징과 Kubernetes 운영 절차는 [apps-gateway-packaging.md](./apps-gateway-packaging.md), Gateway 뒤쪽 proxy 구현은 [apps-gateway-upstream-proxy.md](./apps-gateway-upstream-proxy.md), MCP Gateway는 [mcp-gateway/](./mcp-gateway/)를 따른다.
...
- Bedrock `ApplyGuardrail`을 input prompt에 먼저 적용한다.
...
- 입력 guardrail은 rate limit 통과 후 upstream 호출 전에 실행한다.
- 출력 guardrail은 upstream 응답을 client에 반환하기 전에 실행한다.
- `stream: true` 요청에서 출력 guardrail을 강제하려면 strict mode로 응답을 buffer한 뒤 검사한다.
- Bedrock Guardrails 또는 Redis 장애 시 기본 정책은 fail closed다.
- 상세 구현은 [apps-gateway-upstream-proxy.md](./apps-gateway-upstream-proxy.md)를 따른다.
